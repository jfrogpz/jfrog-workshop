#!/bin/bash
# curation-docker module: task verification functions
# curation-docker 模块：任务验证函数
#
...
verify_curation_docker_T1() {
  local s
  s=$(curl_jf -o /dev/null -w "%{http_code}" \
    "${API}/repositories/${NICKNAME}-docker-curated-virtual" 2>/dev/null || echo "000")
  [ "$s" = "200" ]
}

verify_curation_docker_T2() {
  # Check the remote cache has content (image was pulled through)
  local children
  children=$(curl_jf "${API}/storage/${NICKNAME}-docker-curated-remote" 2>/dev/null \
    | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d.get('children',[])))" \
    2>/dev/null || echo "0")
  [ "$children" -gt 0 ]
}

verify_curation_docker_T3() {
  local found="no"
  local offset=0
  local page_size=50
  while true; do
...
verify_curation_docker_T4() {
  # Check Curation audit for a blocked Docker image pull by this user's repos
  local found="no"
  local offset=0
  local page_size=500
  while true; do
...
try:
    ...
    f = any(
        ...
        'docker' in str(e.get('package_type','')).lower()
        for e in entries
    )
