#!/bin/bash
# xray-npm module: create personal Artifactory repositories for a participant
# xray-npm 模块：为学员创建个人 Artifactory 仓库（启用 Xray 索引）
...
echo "Creating Artifactory repositories for ${NICKNAME} (xray-npm)..."

create_repo "${NICKNAME}-npm-xray-local" \
  "{\"rclass\":\"local\",\"packageType\":\"npm\",\"repoLayoutRef\":\"npm-default\",\"xrayIndex\":true}"

create_repo "${NICKNAME}-npm-xray-remote" \
  "{\"rclass\":\"remote\",\"packageType\":\"npm\",\"url\":\"https://registry.npmjs.org\",\"repoLayoutRef\":\"npm-default\",\"xrayIndex\":true}"

create_repo "${NICKNAME}-npm-xray-virtual" \
  "{\"rclass\":\"virtual\",\"packageType\":\"npm\",\"repoLayoutRef\":\"npm-default\",\"repositories\":[\"${NICKNAME}-npm-xray-local\",\"${NICKNAME}-npm-xray-remote\"],\"defaultDeploymentRepo\":\"${NICKNAME}-npm-xray-local\"}"
