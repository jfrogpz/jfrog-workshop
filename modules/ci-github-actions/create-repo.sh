#!/bin/bash
# ci-github-actions module: create personal Artifactory repositories for a participant
# ci-github-actions 模块：为学员创建个人 Artifactory 仓库
...
echo "Creating Artifactory repositories for ${NICKNAME} (ci-github-actions)..."

create_repo "${NICKNAME}-npm-gha-local" \
  "{\"rclass\":\"local\",\"packageType\":\"npm\",\"repoLayoutRef\":\"npm-default\",\"xrayIndex\":true}"

create_repo "${NICKNAME}-npm-gha-remote" \
  "{\"rclass\":\"remote\",\"packageType\":\"npm\",\"url\":\"https://registry.npmjs.org\",\"repoLayoutRef\":\"npm-default\",\"xrayIndex\":true}"

create_repo "${NICKNAME}-npm-gha-virtual" \
  "{\"rclass\":\"virtual\",\"packageType\":\"npm\",\"repoLayoutRef\":\"npm-default\",\"repositories\":[\"${NICKNAME}-npm-gha-local\",\"${NICKNAME}-npm-gha-remote\"],\"defaultDeploymentRepo\":\"${NICKNAME}-npm-gha-local\"}"
