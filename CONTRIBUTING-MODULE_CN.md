...
- `hint` 和 `hint_cn` 在 `check-progress.sh` 输出中显示，帮助遇到困难的学员
...
`check-progress.sh` 会动态调用：任务 ID `maven-basic-T2` → 函数 `verify_maven_basic_T2`。
...
- 函数可以使用 `NICKNAME`、`JFROG_URL`、`JFROG_TOKEN`、`API`、`curl_jf` — 这些由 `check-progress.sh` 在 source 此文件前设置好
...
- [ ] 运行 `bash automation/check-progress.sh` — 所有任务验证正常
