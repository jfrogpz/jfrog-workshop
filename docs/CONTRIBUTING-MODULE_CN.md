...
- **第一个任务**通常是注册任务——如果是，在 `create-repo.sh` 中将其标记为 `done`，或由 `register.sh` 设置初始状态
...
`check-and-update-progress.sh` 会动态调用：任务 ID `npm-basic-T2` → 函数 `verify_maven_basic_T2`。
...
- [ ] `tasks.json` — 第一个任务的 ID 和结构与 `register.sh` 的预期一致
