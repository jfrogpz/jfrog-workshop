...
Task IDs use the format `<module>-<sequence>`, e.g. `npm-security-T1`. The verification logic for each task lives in `modules/<module>/verify-tasks.sh`. For task-level details, refer to the module's `.github/instructions/<module>.instructions-en.md`.
