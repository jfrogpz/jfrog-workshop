...
1. **T1 注册昵称**：选择一个独特昵称，脚本自动在 Artifactory 上为你创建专属的 npm Artifactory 仓库组
2. **T2 首次构建**：配置 npm 通过你的 Artifactory 虚拟仓库解析依赖，完成 `npm install + build`
3. **T3 发布 Build Info**：将构建的完整依赖信息发布到 Artifactory，建立可追溯性
4. **T4 创建安全策略**：在 JFrog Curation 中创建一条针对你个人 Artifactory 仓库的 npm 风险包拦截策略
