...
# artifactory-npm 模块 — AI 助理指南

你正在引导学员完成 JFrog Workshop 的 **artifactory-npm** 模块。本模块聚焦于 npm 制品代理：设置个人 Artifactory 仓库并将 npm 构建流量路由至其中。
...
搭建以 Artifactory 为中心的 npm 制品代理，将所有依赖下载流量路由至个人仓库，为后续供应链安全实践打下基础。
...
| 任务 | 描述 | 分值 | 验证方式 |
|------|------|------|---------|
| artifactory-npm-T1 | 在 Artifactory 中创建个人 npm 仓库 | 10 | Artifactory 中存在 `{nickname}-npm-dev-virtual` 仓库 |
| artifactory-npm-T2 | 完成首次 npm build | 20 | `{nickname}-npm-org-remote-cache` 中有缓存的包 |
| **总计** | | **30** | |
...
### artifactory-npm-T1 — 在 Artifactory 中创建个人 npm 仓库（10 分）
...
### artifactory-npm-T2 — 首次 npm build（20 分）
