...
任务以**模块**为单位组织（如 `npm-security`、`npm-basic`），每个模块有独立的任务列表和分值。讲师在创建赛事时指定本场活跃的模块。
...
> 🤖 **如果 Copilot Chat 不可用**，直接阅读对应模块的中文指令文件，例如 [.github/instructions/npm-security.instructions-cn.md](.github/instructions/npm-security.instructions-cn.md)，其中包含全部任务的详细步骤和命令。
...
👉 [CONTRIBUTING-MODULE_CN.md](./docs/CONTRIBUTING-MODULE_CN.md)
