...
> 🌐 [English version](./ORGANIZER_EN.md)
...
单模块赛事（`--modules curation-npm`）排行榜效果示例：
...
多模块赛事（`--modules curation-npm,artifactory-npm`），每个模块独立排名，最后汇总总分：
...
> **说明**：列标签显示任务 ID 的最后一段（如 `curation-npm-T1` → `T1`）。每个模块区块按该模块得分独立排名。
...
| 本场启用模块 | 如 `artifactory-npm`、`curation-npm`（即 `--modules` 中指定的模块） |
...
如需调整各任务分值，修改对应模块目录下的 `tasks.json`（如 `modules/curation-npm/tasks.json`），然后重新运行初始化脚本即可覆盖 `config.json`：
...
任务 ID 格式为 `<模块名>-T<序号>`，如 `curation-npm-T1`。各任务的验证逻辑详见对应模块的 `.github/instructions/<module>.instructions.md`。
