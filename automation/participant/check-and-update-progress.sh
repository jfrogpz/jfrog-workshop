...
if [ ! -f "$PROFILE_FILE" ]; then
  ...
  echo "  Please register first / 请先运行注册脚本：bash automation/participant/register.sh <NICKNAME> [EVENT_ID]" >&2
  echo "" >&2
  exit 1
fi
...
for module in sorted(os.listdir(modules_dir)):
    ...
    if os.path.isfile(tf):
        ...
        tasks.extend([{'id': t['id'], 'name': t.get('name',''), 'name_cn': t.get('name_cn',''), 'points': t['points'], 'hint': t.get('hint',''), 'hint_cn': t.get('hint_cn',''), 'verify': t.get('verify', True)} for t in mt])
...
# Build set of task IDs that opt out of automated verification
NO_VERIFY_IDS=$(echo "$TASKS_JSON" | python3 -c "
import sys, json
tasks = json.load(sys.stdin)
for t in tasks:
    if t.get('verify') is False:
        print(t['id'])
" 2>/dev/null || echo "")
...
"
  elif echo "$NO_VERIFY_IDS" | grep -qxF "$TASK_ID"; then
    # verify: false — auto-pass, no programmatic check needed
    VERIFY_RESULTS="${VERIFY_RESULTS}${TASK_ID}=pass
"
  else
